...
%   set(fig10,'Position',[0 0 1131 813]);
  ...
  p1 = plotSigEp(dat, marker(1,:), color(1,:), fid, sig0(1));
  
  delT = 10;
  epdot = 5.7e-4;
  T = 77.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(1,:), color(1,:));
  
  load SigEpsEp000057s201KOga.dat.raw
  dat = SigEpsEp000057s201KOga;
  p2 = plotSigEp(dat, marker(2,:), color(2,:), fid, sig0(2));
  
  delT = 10;
  epdot = 5.7e-4;
  T = 201.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(2,:), color(2,:));
  
  load SigEpsEp000057s289KOga.dat.raw
  dat = SigEpsEp000057s289KOga;
  p3 = plotSigEp(dat, marker(3,:), color(3,:), fid, sig0(3));

  delT = 10;
  epdot = 5.7e-4;
  T = 289.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(3,:), color(3,:));
  
  load SigEpsEp0000048s297KHog.dat.raw
  dat = SigEpsEp0000048s297KHog;
  p4 = plotSigEp(dat, marker(4,:), color(4,:), fid, sig0(4));

  delT = 10;
  epdot = 4.8e-5;
  T = 297.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(4,:), color(4,:));
  
  load SigEpsEp000057s373KOga.dat.raw
  dat = SigEpsEp000057s373KOga;
  p5 = plotSigEp(dat, marker(5,:), color(5,:), fid, sig0(5));
  
  delT = 10;
  epdot = 5.7e-4;
  T = 373.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(5,:), color(5,:));

  load SigEpsEp000057s473KOga.dat.raw
  dat = SigEpsEp000057s473KOga;
  p6 = plotSigEp(dat, marker(6,:), color(6,:), fid, sig0(6));

  delT = 10;
  epdot = 5.7e-4;
  T = 473.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(6,:), color(6,:));
  
  load SigEpsEp0000010s533KHog.dat.raw
  dat = SigEpsEp0000010s533KHog;
  p7 = plotSigEp(dat, marker(7,:), color(7,:), fid, sig0(7));

  delT = 10;
  epdot = 5.7e-4;
  T = 533.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(7,:), color(7,:));
  
  load SigEpsEp0000010s644KHog.dat.raw
  dat = SigEpsEp0000010s644KHog;
  p8 = plotSigEp(dat, marker(8,:), color(8,:), fid, sig0(8));
  
  delT = 10;
  epdot = 5.7e-4;
  T = 644.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(8,:), color(8,:));
...
function plot0001Edot(fid, marker, color)
  ...
  p9 = plotSigEp(dat, marker(9,:), color(9,:), fid, sig0(9));
  
  delT = 5;
  epdot = 1.0e-3;
  T = 478.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(9,:), color(9,:));

  load SigEpsEp0001s478KEle.dat.raw
  dat = SigEpsEp0001s478KEle;
  p10 = plotSigEp(dat, marker(10,:), color(10,:), fid, sig0(10));
  
  delT = 5;
  epdot = 1.0e-3;
  T = 478.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(10,:), color(10,:));

  load SigEpsEp0001s561KEle.dat.raw
  dat = SigEpsEp0001s561KEle;
  p11 = plotSigEp(dat, marker(11,:), color(11,:), fid, sig0(11));
  
  delT = 5;
  epdot = 1.0e-3;
  T = 561.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(11,:), color(11,:));

  load SigEpsEp0001s589KGre.dat.raw
  dat = SigEpsEp0001s589KGre;
  p12 = plotSigEp(dat, marker(12,:), color(12,:), fid, sig0(12));
  
  delT = 5;
  epdot = 1.0e-3;
  T = 589.0;
  epmax = max(dat(:,5));
  adiMTS(epdot, T, delT, epmax, marker(12,:), color(12,:));
...
%   adiMTS(epdot, T, delT, epmax, marker(6,:), color(6,:));
  ...
  epmax = max(dat(:,5));
  rhomax = 2690.0
  adiMTS(epdot, T, delT, epmax, marker(7,:), color(7,:), rhomax);
  ...
  epmax = max(dat(:,5));
  rhomax = 2692;
  adiMTS(epdot, T, delT, epmax, marker(8,:), color(8,:), rhomax);
...
function adiMTS(epdot, T0, delT, epmax, marker, color, rhomax)
  ...
  delrho = (rho0-rhomax)/m;
  rho = rho0-0.1;
  T = T0;
  ep = 0.0;
%   Cp = 950;
  for i=1:m
    sig(i) = MTS(epdot, ep, T, T0, rho, rho0);
    eps(i) = ep;
    ep = ep + epdot*delT;
    Cp = calcCp(T)
    fac = 0.9/(rho*Cp);
...
p  = plot(eps, sig*1.0e-6, 'k-.', 'LineWidth', 2); hold on;
set(p,'LineWidth',2,'Color',color,'MarkerSize',8,'Marker',marker);
...
function [sigma_e] = computeSigma_e(theta_0, sigma_es, ep)
  ...
  alpha = 2.0;
  A = theta_0*ep;
  B = 0.5*(theta_0*sigma_es*log(tanh(alpha*ep/sigma_es)-1))/(tanh(alpha)*alpha);
  C = 0.5*(theta_0*sigma_es*log(tanh(alpha*ep/sigma_es)+1))/(tanh(alpha)*alpha);
  sigma_e = A + B + C;
